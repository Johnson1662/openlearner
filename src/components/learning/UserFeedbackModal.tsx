      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <motion.div
            ...
            className="relative bg-white rounded-3xl max-w-md w-full mx-4 overflow-hidden border-2 border-duo-gray"
            style={{ boxShadow: '0 8px 0 #E5E5E5' }}
          >
            {/* Header */}
            <div className="px-8 pt-8 pb-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-2xl font-black text-duo-text">课程反馈</h2>
                <button
                  onClick={handleSkip}
                  className="w-8 h-8 flex items-center justify-center rounded-xl cursor-pointer bg-muted"
                >
                  <X className="w-4 h-4 text-duo-grayDark" />
                </button>
              </div>
              <p className="font-semibold text-duo-grayDark">
            ...
            <div className="px-8 pb-6">
              <label className="block text-sm font-extrabold uppercase tracking-wide mb-2 text-duo-text">
                其他建议（可选）
              </label>
              <textarea
                value={feedbackText}
                ...
                className="w-full h-24 px-4 py-3 rounded-2xl border-2 font-semibold resize-none focus:outline-none transition-all border-duo-gray text-duo-text"
            ...
            <div className="px-8 pb-8 flex gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.97, y: 2 }}
                onClick={handleSkip}
                className="flex-1 py-4 rounded-2xl border-2 font-extrabold uppercase tracking-wider cursor-pointer border-duo-gray text-duo-grayDark bg-muted"
              ...
              <motion.button
                ...
                className="flex-1 py-4 rounded-2xl font-black uppercase tracking-wider text-white flex items-center justify-center gap-2 cursor-pointer bg-primary"
                style={{ boxShadow: '0 4px 0 #6B48D4', border: '2px solid #6B48D4' }}
              >
