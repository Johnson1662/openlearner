...
export default function Segmenter({ 
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div 
        className="relative flex items-center bg-muted p-8 rounded-[24px] border-2 border-duo-gray shadow-inner"
        ...
        <div className="flex items-center gap-0">
          {chars.map((char, i) => (
            <div key={i} className="flex items-center">
              <motion.div 
                className="text-4xl font-black text-duo-text px-1"
                layout
              >
                {char}
              </motion.div>
                  ...
                  <motion.div 
                    className={`
                      ...
                      ${markers.includes(i) 
                        ...
                        : 'bg-primary/10 opacity-0 group-hover:opacity-50'}
                    `}
                    initial={false}
        ...
        <AnimatePresence mode="popLayout">
          {currentSegments.map((seg, i) => (
            <motion.div
              ...
              className="px-6 py-3 bg-background border-2 border-duo-gray rounded-[16px] shadow-[0_4px_0_0_#E5E5E5] text-xl font-black text-duo-text"
            >
              {seg}
            </motion.div>
          ))}
      ...
      <p className="mt-8 text-duo-textMuted font-bold text-sm uppercase tracking-widest">
